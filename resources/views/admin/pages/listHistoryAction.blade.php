...
@section('content')
                        ...
                        <div class='modal-content portlet box blue-hoki'>
                            ...
                            <div class="modal-body portlet-body" id="form-action">
                                            <table  class="table borderless" style="border-style: none">
                                                <tr>
                                                  <td class="col-xs-1"><label class="control-label">{{trans('history_action.user')}}</label></td>
                                                  <td class="col-md-2"><select class="form-control input-medium select2me" position="1" name="user_id" id="user_id" data-placeholder="{{trans('menu.origin')}}">
                                                                           @foreach($user as $u)
                                                ...
                                                <tr>
                                                  <td class="col-xs-1"><label class="control-label">{{trans('history_action.type')}}</label></td>
                                                  <td class="col-md-2"><select class="form-control input-medium select2me" position="2" name="type" id="type" data-placeholder="{{trans('menu.origin')}}">
                                                                            <option value="0">{{trans('menu.origin')}}</option>
                                                                            <option value="1">{{trans('login.sign_in')}}</option>
                                                                            <option value="2">{{trans('global.add')}}</option>
                                                                            <option value="3">{{trans('global.copy')}}</option>
                                                                            <option value="4">{{trans('global.edit')}}</option>
                                                                            <option value="5">{{trans('global.delete')}}</option>
                                                                            <option value="6">{{trans('global.import')}}</option>
                                                                    </select>  </td>     
                                                </tr>        
                                                <tr>
                                                  <td class="col-xs-1"><label class="control-label">{{trans('history_action.name')}}</label></td>
                                                  <td class="col-md-2"><select class="form-control input-medium select2me" position="3" name="menu_id" id="menu_id" data-placeholder="{{trans('menu.origin')}}">
                                                                           @foreach($menu as $m)
                                                                           <option value="{{$m->id}}">{{trans('menu.'.$m->code)}}</option>
                                                                           @endforeach
                                                                    </select> </td>     
                                                </tr>                                               
                                                <tr>                                      
                                                  <td class="col-xs-1"><label class="control-label">{{trans('history_action.content')}}</label></td>
                                                  <td class="col-md-2"><textarea name="content" position="4" class="ckeditor"></textarea></td>     
                                                </tr>                                               
                                                <tr>                                        
    ...
    <div class="modal fade" id="import" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-body">                                       
                                        ...
                                        <div class="tr-space"></div>
                                            <div class="form-group">
                                                    ...
                                                    <div class="col-md-5">
                                                        <a class="btn default" href="{{url(session()->get('locale').'/'.session()->get('manage').'/export/template')}}" >{{trans('global.download')}}</a>
                                                    </div>
                                            </div>
                                        <div class="tr-space"></div>
                                        <div class="tr-space"></div>
                  ...
                  <div class="portlet light bordered">
                                <div class="portlet-title tabbable-line">
                                    <div class="caption">
                                        ...
                                        <span class="caption-subject font-purple bold uppercase">{{trans('menu.history-action')}}</span>
                                    </div>
                                                ...
                                                <div class="portlet-body col-md-12">                                                        
                                                        <table class="table table-striped table-bordered table-hover order-column context-menu" style="border:none" id="grid">
                                                            <thead>
                                                                ...
                                                                <th> {{trans('history_action.user')}}</th>
                                                                <th> {{trans('history_action.type')}}</th>
                                                                <th> {{trans('history_action.name')}}</th>
                                                                <th> {{trans('history_action.content')}}</th>
        ...
        jQuery(document).ready(function() {   
           ...
           EposAdminModal.url = <?= json_encode(['save_url'=>'add/history-action','delete_url'=>'update/history-action','import_url'=>'import/history-action']);?>;
        });   
        </script>
