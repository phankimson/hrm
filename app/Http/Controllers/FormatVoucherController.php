...
use App\Http\Models\FormatVoucher;
use App\Classes\Helpers;
use Excel;
use DB;

class FormatVoucherController extends Controller
{
    private $data;
    private $column;
       public function __construct()
    {
        $this->data = FormatVoucher::all();
        $column  = DB::getSchemaBuilder()->getColumnListing('format_vouchers');
        $this->column = array_diff($column, array("id", "created_at","updated_at"));
    }

  public function showPage(){
        return view("admin.pages.listFormatVoucher",['data'=>$this->data]);
    }
  public function save(Request $request){
        ...
        $t = json_decode($data);        
            ...
            $return = new FormatVoucher();
             }else{
            $return = FormatVoucher::find($t->id);
            }
            foreach($colum as $value){
                   $return->$value           = Helpers::get_not_null($t-> $value,0); 
            }            
            $return->save();
            $lst_data =  FormatVoucher::get_all($return->id);     
     ...
     public function export(){   
       Excel::create('ExcelFormImport', function ($excel) {
        ...
        $excel->sheet('FormatVoucher', function ($sheet) {
...
//            }
    ...
    public function import(Request $request){
          ...
          foreach($sheet as $sh){  
                ...
                $return -> save();
                $data[$count] = FormatVoucher::get_all($return->id);  
