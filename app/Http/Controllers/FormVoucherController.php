...
use App\Http\Models\FormVoucher;
...
class FormVoucherController extends Controller
{
    ...
    {
        $this->data = FormVoucher::all();
        $column  = DB::getSchemaBuilder()->getColumnListing('form_voucher');
        $this->column = array_diff($column, array("id", "created_at","updated_at"));
    }

  public function showPage(){
        return view("admin.pages.listFormVoucher",['data'=>$this->data]);
    }
  public function save(Request $request){
        ...
        $t = json_decode($data);        
            ...
            $return = new FormVoucher();
             }else{
            $return = FormVoucher::find($t->id);
            }
            foreach($colum as $value){
                   $return->$value           = Helpers::get_not_null($t-> $value,0); 
            }            
            $return->save();
            $lst_data =  FormVoucher::get_all($return->id);     
     ...
     public function export(){   
       Excel::create('ExcelFormImport', function ($excel) {
        ...
        $excel->sheet('FormVoucher', function ($sheet) {
...
//            }
    ...
    public function import(Request $request){
          ...
          foreach($sheet as $sh){  
                ...
                $return -> save();
                $data[$count] = FormVoucher::get_all($return->id);  
