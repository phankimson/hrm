                ...
                $print.click(function () {
                    function buildTable(data,columns) {
                        ...
                        for(var h = 0; h < columns.length; h++) {
                            out += "<tr>";
                            for(var k = 0; k < columns[h].length;k++){
                                if(!columns[h][k].printIgnore) {
                                    out += ("<th colspan="+columns[h][k].colspan+" rowspan="+columns[h][k].rowspan+">"+columns[h][k].title+"</th>");
                                    }
                                }
                            out += "</tr>";    
                        }
                        ...
                        for(var i = 0; i < data.length; i++) {
                            ...
                            for(var j = 0; j < columns.length; j++) {
                                for(var l = 0 ; l < columns[j].length; l++){                                    
                                    if(!columns[j][l].printIgnore && columns[j][l].field) {
                                        if(columns[j][l].formatter){
                                         out += ("<td>"+eval(columns[j][l].formatter)(data[i][columns[j][l].field]||"-")+"</td>");    
                                        }else{
                                        out += ("<td>"+(data[i][columns[j][l].field]||"-")+"</td>");
                                     }
                                    }
                                }                           
                            }
